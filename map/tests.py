...
from map.views import build_tag_list
...
class LocationTestCase(TestCase):
    ...
    def test_objects_properly_created(self):
        ...
        self.assertEqual(regular.is_public, True)
        ...
        self.assertEqual(zero.is_public, True)
        ...
        self.assertEqual(tags.is_public, True)
    ...
    def test_build_tag_list(self):
        location_list = [list(Location.objects.
                              filter(title='Test regular values').values())[0],
                         list(Location.objects.
                              filter(title='Test near zero').values())[0],
                         list(Location.objects.
                              filter(title='Test for tags').values())[0]]
        result_1 = [{'name': 'drei_test',
                     'included': True},
                    {'name': 'ein_test',
                     'included': False},
                    {'name': 'exklusiv',
                    ...
                    {'name': 'zwei_test',
                     'included': True}]
        self.assertEqual(build_tag_list(location_list,
                                        'test1,test2,zwei_test,drei_test'),
                         result_1)
